...
public class JarvisMarch extends ConvexHull
{
     ...
     * 
     *     1) The array pointsNoDuplicate[] contains just one point, in which case the convex
	...
	{
		// Condition for if length is 1 or 2
		...
		int mid = rightChain.size();
		
		// Copy stacks to hullVertices
		for(int i = rightChain.size() - 1; i >= 0; i--){
			hullVertices[i] = rightChain.pop();
		}
		for(int i = hullVertices.length - 1; i >= mid; i--){
	...
	public Point nextVertex(Point v)
	{
		...
		quicksorter.quickSort(new PolarAngleComparator(v, true));
		...
		for(int i = 0; i < pointsNoDuplicate.length - 1; i++){
			if(pointsNoDuplicate[i].equals(v)){
				i++;
				n = pointsNoDuplicate[i];
				i = pointsNoDuplicate.length;
			}
		}
